...
//std::string Rootpath = "../../.common/data/";

float Starttime = 0.00f;
 float Timestep = 0.01f;    //unit:days
int N = 2;       // unit : a round of earth orbit
int Repeats = int(365 * N / Timestep);  //# unit : days
...
enum class CameraID : const unsigned int
{
	SolarCamera,
	EarthCamera,
	MoonCamera,
...
MyCamera::Camera2* cameraPtr{nullptr};

SolarSys::SolarMotionData* solarMotionData;
...
Switch g_writeSwitchCamera{ Switch::Off };
Switch g_writeSwitchMotion{ Switch::Off };
Switch g_showCameraPositions{ Switch::Off };
...
ViewPort4 g_port[static_cast<int>(CameraID::Count)] =
{ ViewPort4::TopLeft, ViewPort4::BottomLeft, ViewPort4::BottomRight };
...
void projection(int w, int h, ProjectionType projectiontype = ProjectionType::Pers);
void drawText(GLuint shaderprogram);
void callback_DrawStars();
...
void initializing() {
	...
	SolarSys::initialSetup(constantCSV, shaderCSV, textureCSV);
...
void resetCamera() {
	...
	cameraPtr = new MyCamera::Camera2{ eye, front, up };
...
int main() {
	...
	solarMotionData = new SolarSys::SolarMotionData{Starttime, Timestep, Repeats, SolarSys::DifferentialAlgorism::rungekutta, SolarSys::constant};
	...
	while (!glfwWindowShouldClose(window)) {
		...
		solarMotionData->drawStars(callback_DrawStars);

		solarMotionData->updateMotionData(false);  // true for debug
...
void callback_DrawStars() {
	...
	// Draw stars
	std::vector<SolarSys::MotionDataFormat> LastMember = solarMotionData->motionDataContainer.back();
	for (SolarSys::MotionDataFormat& star : LastMember) {
		solarTime = star.systimer; // note: same in the datalist, which consist of motion data with same systimer 
		...
		// Transform corrdinates XYZ to Opengl coordinates ZXY
		//glm::vec4 XYZ{ star.scaledPosition, 0.0f };
		//glm::vec4 ZXY = glm::rotateZ(XYZ, glm::radians(90.0f));
		//ZXY = glm::rotateX(ZXY, glm::radians(-90.0f));
		//modelMat = glm::translate(modelMat, glm::vec3(ZXY));// OPenGL coordinates
		modelMat = glm::translate(modelMat, star.scaledPosition);// OPenGL coordinates
		modelMat = glm::scale(modelMat, glm::vec3(size));
	...
	// Draw tail line
	LineVertices tailLine{};
	/*glm::vec4 XYZ{};
	glm::vec4 ZXY{};*/
	glm::vec4 lineColor{ 1.0f, 1.0f, 1.0f, 1.0f };
	for (int planetid = 0; planetid < solarMotionData->Constant.size(); planetid++) {
		shaderName = "line";
		tailLine.segments.clear();
		tailLine.indices.clear();
		int j{0};
		//for (int i = 0; i < solarMotionData->tailLineLength - 1; i++) {
		for (int i = solarMotionData->tailLineLength - 1; i > 0; --i) {
			// Transform corrdinates XYZ to Opengl coordinates ZXY
			/*XYZ = glm::vec4{ solarMotionData->motionDataContainer[i][planetid].scaledPosition, 0.0f };
			ZXY = glm::rotateZ(XYZ, glm::radians(90.0f));
			ZXY = glm::rotateX(ZXY, glm::radians(-90.0f));
			tailLine.segments.push_back({ ZXY, lineColor });*/
			tailLine.segments.push_back({ solarMotionData->motionDataContainer[i][planetid].scaledPosition, lineColor });
			//tailLine.segments.push_back({ XYZ, lineColor });
			// Transform corrdinates XYZ to Opengl coordinates ZXY
			j = i - 1;
			/*XYZ = glm::vec4{ solarMotionData->motionDataContainer[j][planetid].scaledPosition, 0.0f };
			ZXY = glm::rotateZ(XYZ, glm::radians(90.0f));
			ZXY = glm::rotateX(ZXY, glm::radians(-90.0f));
			tailLine.segments.push_back({ ZXY, lineColor });*/
			tailLine.segments.push_back({ solarMotionData->motionDataContainer[j][planetid].scaledPosition, lineColor });

			//tailLine.segments.push_back({ XYZ, lineColor});
			tailLine.indices.push_back(i);
			tailLine.indices.push_back(j);
		}
		...
		bindBufferLines(tailLine, VAOs[LineBuff], VBOs[LineBuff], EBOs[LineBuff]);
		...
		drawElements(VAOs[LineBuff], VBOs[LineBuff], EBOs[LineBuff], GL_LINES, tailLine.indices.size(), &tailLine.indices[0]);
		glDisable(GL_LINE_SMOOTH);
		glUseProgram(0);
	...
	projection(g_displayWidth, g_displayHeight, ProjectionType::Ortho);
	drawText(SolarSys::shader.at("text").shaderPtr->ID);


	//solarMotionData->updateMotionData();
...
void drawText(GLuint shaderprogram) {
	...
	snprintf(textbuffer[5], static_cast<size_t>(bufSize), "Camera Up = ( %#7.2f, %#7.2f, %#7.2f)", cameraPtr->Up.x, cameraPtr->Up.y, cameraPtr->Up.z);
	snprintf(textbuffer[6], static_cast<size_t>(bufSize), "Camera Target = ( %#7.2f, %#7.2f, %#7.2f)", (cameraPtr->Position+radius * cameraPtr->Front).x, (cameraPtr->Position + radius * cameraPtr->Front).y, (cameraPtr->Position + radius * cameraPtr->Front).z);
	snprintf(textbuffer[7], static_cast<size_t>(bufSize), "Camera Yaw, Pich = ( %#7.2f, %#7.2f)", cameraPtr->Yaw, cameraPtr->Pitch);
	snprintf(textbuffer[8], static_cast<size_t>(bufSize), "Camera Fov = %#4.1f", cameraPtr->Fov);
...
//	switch (key) {
//	case GLFW_KEY_KP_0: {
//		g_timerControl = TimerControl::Stop;
//		break;
//	}
//	case GLFW_KEY_KP_1: {
//		g_timerControl = TimerControl::Slower;
//		break;
//	}
//	case GLFW_KEY_KP_2: {
//		g_timerControl = TimerControl::Normal;
//		break;
//	}
//	case GLFW_KEY_KP_3: {
//		g_timerControl = TimerControl::Fast;
//		break;
//	}
//	case GLFW_KEY_KP_4: {
//		g_timerControl = TimerControl::Faster;
//		break;
//	}
//	case GLFW_KEY_HOME: {
//		resetSolarSystem();
...
void keyboard_CameraPosition(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	eye = calcEyePosition(radius, theta, phi);
	cameraPtr->Position = eye;
	//cameraPtr->Front = glm::normalize(-eye);
...
CameraID  checkCursorPos(int x, int y) {
	...
	CameraID screen{ CameraID::EarthCamera };
	int x1{ 0 }, x2{ 0 }, y1{ 0 }, y2{ 0 };

	for (int camera = static_cast<int>(CameraID::SolarCamera); camera < static_cast<int>(CameraID::Count); camera++) {
		...
		if (x1 < X && X < x2 && y1 < Y && Y < y2) {
			screen = static_cast<CameraID>(camera);
